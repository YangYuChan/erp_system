  ...
  <div class="table">
    ...
    <el-form :inline="true" :model="listQuery" class="demo-form-inline">
      <el-form-item label="筛选条件">
        <el-select v-model="listQuery.staffId" placeholder="选择销售" :clearable=true  @change="handleFilter">
          <el-option v-for="item in staffData" :key="item.id" :label="item.name" :value="item.id"></el-option>
        </el-select>
        <el-select v-model="listQuery.payType" placeholder="选择支付方式" :clearable=true @change="handleFilter">
    ...
    <el-table :data="tableData" border stripe style="width:100%" v-loading="listLoading" id="exportTable">
      ...
      <el-table-column prop="payType" label="支付方式">
        <template slot-scope="scope">
          ...
          <span v-else-if="scope.row.payType == 4">pos机</span>
          <span v-else-if="scope.row.payType == 5">银联</span>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      handleExport(){
        this.downloadUrl = this.path + 'erp/card/sale/excel?kindId='+this.listQuery.kindId
          +'&kindType='+this.listQuery.kindType
          +'&startTime='+this.listQuery.startTime
          +'&endTime='+this.listQuery.endTime
          +'&staffId='+this.listQuery.staffId
        ...
        xhr.onload = function (e) {
          if (this.status == 200) {
            ...
            link.download = year+month+dates+'会员卡销售报表.xls';
            link.click()
          } else {
