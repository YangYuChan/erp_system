  ...
  <div class="table">
    ...
    <div class="subTips clearfix">
      ...
      <div class="btn-group fl-right">
          ...
          <el-button type="default" icon="el-icon-view" @click="handleDetails">查看预约人</el-button>
                ...
                <tr v-for="item in classData">
                  ...
                  <td v-for="pitem in item.parent">
                    <div class="class_item" v-for="citem in pitem.child" @click="selectedClass(citem.id,citem)">
                      ...
                      <p>余量：<span>{{citem.spareNumber}}</span></p>
                    </div>
                  </td>
                </tr>
                <tr v-show="dataState">
    ...
    <!--查看预约人-->
    <el-dialog :visible.sync="classOrderVisible" width="500px" title="查看预约人" class="details-container"
               :closeOnClickModal="false">
      <div class="details-box">
        <p class="tags">(温馨提示：可在列表中编辑时间，自动销课)</p>
        <dl class="class-info">
          <dd class="clearfix">
            <span class="class-name">课程名称：{{classObj.className}}</span>
            <span class="class-price" v-if="classObj.classPrice == 0">免费</span>
            <span class="class-price" v-else>￥{{classObj.classPrice}}</span>
          </dd>
          <dd class="class-time">上课时间：{{classObj.formatBeginEndTime}}</dd>
          <dd class="class-coach">上课教练：{{classObj.coachName}}</dd>
          <dd class="class-limit clearfix">
            <span v-if="classObj.numberLimit == 0 || classObj.numberLimit == null">课程容量：不限</span>
            <span v-else>课程容量：{{classObj.numberLimit}}人</span>
            <span class="last-limit" v-if="classObj.numberLimit == 0 || classObj.numberLimit == null">剩余：不限</span>
            <span class="last-limit" v-else> 剩余：{{classObj.spareNumber}}人</span>
          </dd>
        </dl>
        <div class="order-list">
          <h4>已预约会员</h4>
          <ul v-show="orderList">
            <li class="order-item clearfix" v-for="item in orderMemberList">
              <span>{{item.name}}</span>
              <span v-if="item.sex == 0">男</span>
              <span v-if="item.sex == 1">女</span>
              <span v-if="item.sex == 2">保密</span>
              <span>{{item.phone}}</span>
              <span><el-button type="primary" size="mini" @click="cancelOrder(item)">取消预约</el-button></span>
            </li>
          </ul>
          <p class="noOrder" v-show="noOrder">该课程暂时没有会员预约 !</p>
        </div>
      </div>
    ...
    <el-dialog
      ...
      <dl class="pay-success-info">
        <dd class="info-item"><span>购买日期：</span><span>{{closeOrderInfo.createTime}}</span></dd>
        <dd class="info-item"><span>操作人：</span><span>{{closeOrderInfo.operator}}</span></dd>
        <dd class="info-item"><span>会员：</span><span>{{closeOrderInfo.name}}</span></dd>
        <dd class="info-item"><span>合同编号：</span><span>{{closeOrderInfo.contractNo}}</span></dd>

        <dd class="info-item"><span>团课名称：</span><span>{{closeOrderInfo.tradeDetail}}</span></dd>
        <dd class="info-item"><span>退订数量：</span><span>{{closeOrderInfo.amount}}课时</span></dd>
        <dd class="info-item"><span>实收金额：</span><span>￥{{closeOrderInfo.orderFee}}</span></dd>
        <dd class="info-item"><span>支付方式：</span>
          <span v-if="closeOrderInfo.payMethod == 1">微信支付</span>
          <span v-if="closeOrderInfo.payMethod == 2">支付宝支付</span>
          <span v-if="closeOrderInfo.payMethod == 3">现金支付</span>
          <span v-if="closeOrderInfo.payMethod == 4">pos机支付</span>
          <span v-if="closeOrderInfo.payMethod == 5">银联支付</span>
          <span v-if="closeOrderInfo.payMethod == 6">其他方式支付</span>
        </dd>
        <dd class="info-item"><span>订单号：</span><span>{{closeOrderInfo.tradeNo}}</span></dd>
        <dd class="ddhr"></dd>
        <dd class="info-item clearfix">
          <p><span>退款金额：</span><span class="fl-right"><el-input v-model="refunFee" @change="refunFeeChange(refunFee)" style="width: 100px;"></el-input></span></p>
        <p v-show="remark" class="valid">退款金额不能大有实收金额</p>
        </dd>
        <dd class="info-item"><span>退款方式：</span>
          <span v-if="closeOrderInfo.payMethod == 1">微信支付</span>
          <span v-if="closeOrderInfo.payMethod == 2">支付宝支付</span>
          <span v-if="closeOrderInfo.payMethod == 3">现金支付</span>
          <span v-if="closeOrderInfo.payMethod == 4">pos机支付</span>
        ...
        <dd class="info-item"><span>备注：</span><span>{{closeOrderInfo.note}}</span></dd>
      </dl>
      <div class="btn-box">
        <el-button type="primary" @click="forceCancelOrder()">确 认</el-button>
      </div>
    </el-dialog>
    <!--end-->
  </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      refunFeeChange(value){
          let p = this.closeOrderInfo.orderFee;
          if(value > p) {
            this.remark = true;
          }else{
            this.remark = false;
...
//			},
      ...
      addSubmitForm() {
        ...
        console.log(this.scheduleDetails7);
        let s1 = this.scheduleDetails1.classId,
            s2 = this.scheduleDetails2.classId,
            s3 = this.scheduleDetails3.classId,
            s4 = this.scheduleDetails4.classId,
            s5 = this.scheduleDetails5.classId,
            s6 = this.scheduleDetails6.classId,
            s7 = this.scheduleDetails7.classId;
...
//				this.getOtherCoachList();
      ...
      handleDetails() {
        if (this.classObj == null || this.classObj == '') {
          ...
          this.getOrderMember(this.selectedId);
        }
      },
      //获取已预约的会员列表
      getOrderMember(id) {
        ...
        this.$axios({
          url: this.path + 'class/schedule/member/list?classScheduleId='+id,
          method: 'get',
          headers: {
            'Content-Type': 'application/x-www-form-urlencoded',
            'Authorization': this.token,
          ...
          .then((res) => {
              if(res.data.code == 100000){
                if(res.data.list.length > 0){
                  this.orderMemberList = res.data.list;
                }else{
                  this.noOrder = true;
                  this.orderList = false;
                }
                this.classOrderVisible = true;
              }
          })
          .catch((err) => {
            console.log(err);
      ...
      getOrderPrint(tradeId,memberId){
        this.$axios({
          ...
          .then((res) => {
            if(res.data.code === 100000) {
              this.orderFee = res.data.order.orderFee;
            }
          })
          .catch((err) => {
            console.log(err);
      ...
      cancelOrder(obj){
        //免费课程
        this.cancelOrderId = obj.id;
        this.getOrderPrint(obj.id,obj.memberId)
        if(this.orderFee == 0 || this.orderFee == undefined || this.orderFee == null){
          this.$confirm('确定取消'+obj.name+'的预约课程吗?', '提示', {
            confirmButtonText: '取消预约',
            cancelButtonText: '点错了',
            type: 'warning'
          }).then(() => {
            this.$axios({
              url: this.path + 'class/schedule/relative/cancel',
              method: 'post',
              data: {
                relativeId: obj.id,
                isForce: 0,
          ...
          this.$confirm(obj.name+'已为此课程支付'+this.orderFee+'元，取消预约会产生订单退订', '提示', {
            ...
            this.$axios({
              ...
              headers: {
                ...
                if (res.data.code != 100000) {
                  this.closeOrderInfo = res.data.order;
                  this.refunFee = this.closeOrderInfo.orderFee;
      ...
      forceCancelOrder(){
        if(this.remark == false){
          this.$axios({
            ...
            method: 'post',
            data: {
              relativeId: this.cancelOrderId,
              isForce: 1,
              refunFee:this.refunFee
            },
            transformRequest: [function (data) {
              let ret = ''
              for (let it in data) {
