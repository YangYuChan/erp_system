<template>
	<div class="table">
		...
		<el-form :inline="true" :model="listQuery" class="demo-form-inline">
			...
			<el-form-item>
				<el-select v-model="listQuery.status" placeholder="会员卡状态" :clearable=true @change="handleFilter">
			...
			<div v-show="moreFilter">
				<el-form-item label="筛选条件">
					...
					<el-select v-model="listQuery.kindId" placeholder="会员卡名称" :clearable=true @change="handleFilter">
      ...
      <el-table-column prop="contractStatus" label="合同类型">
        <template slot-scope="scope">
          <span v-if="scope.row.contractStatus == 0">新办</span>
          <span v-else-if="scope.row.contractStatus == 1">转让</span>
        </template>
      </el-table-column>
      ...
      <el-table-column prop="status" label="会员卡状态">
        <template slot-scope="scope">
			...
			<el-table-column label="操作" width="350">
				<template slot-scope="scope">
					<span v-if="scope.row.status == 2">
						<el-button size="small" disabled="disabled">编辑</el-button>
	          <el-button size="small" @click="goToClassPrint(scope.row)">打印小票预览</el-button>
	          <el-button size="small" disabled="disabled">退款</el-button>
	          <el-button size="small" disabled="disabled">转让</el-button>
					</span>
					<span v-else-if="scope.row.status == 3">
          ...
          <el-button size="small" type="danger" @click="handleTransfer(scope.row)">转让</el-button>
  ...
  <!--退款-->
    <el-dialog :visible.sync="offMoneyVisible" title="私教课退款" width="550px"  :before-close="handleMoneyClose" :closeOnClickModal="false" class="details-container">
      <div class="details-box">
        <el-form ref="exitMoneyForm" :model="exitMoneyForm" label-width="100px" :rules="exitMoneyRules"  class="form-container">
          <div class="gray-bg">
            <div class="clearfix">
              <el-form-item label="购买人：" prop="name" class="exit-form">
                <span>{{classObj.name}}</span>
              </el-form-item>
              <el-form-item label="手机号：" prop="phone" class="exit-form">
                <span>{{classObj.phone}}</span>
              </el-form-item>
            </div>
            <div class="bline clearfix">
              <el-form-item label="销售：" prop="staffName" class="exit-form">
                <span>{{classObj.staffName}}</span>
              </el-form-item>
            </div>

            <el-form-item label="会员卡：" prop="kindName" style="margin-bottom: 0">
              <span>{{classObj.kindName}}</span>
            </el-form-item>
            <div class="clearfix">
              <el-form-item label="会员卡号：" prop="cardNum"  class="exit-form">
                <span>{{classObj.cardNum}}</span>
              </el-form-item>
              <el-form-item label="是否激活：" prop="isActivate"  class="exit-form">
                  <span v-if="classObj.isActivate == 0">未激活</span>
                  <span v-else-if="classObj.isActivate == 1">激活成功</span>
              </el-form-item>
            </div>

            <el-form-item label="价格：" prop="money"  style="margin-bottom: 0">
              <span>{{exitMoneyForm.money}}元</span>
              <el-input v-model="exitMoneyForm.money" type="hidden"></el-input>
            </el-form-item>
          ...
          <el-form-item label="退款：" prop="num">
            <el-input v-model.number="exitMoneyForm.num" placeholder="请输入金额"></el-input>&nbsp;&nbsp;元
          </el-form-item>
          ...
          <el-form-item label="备注：" prop="remark" >
            <el-input v-model="exitMoneyForm.remark" type="textarea"></el-input>
          </el-form-item>
          <el-form-item>
    ...
    <!--转让-->
    <el-dialog :visible.sync="transferVisible" title="会员卡转让" width="550px"  :before-close="handleTransferClose" :closeOnClickModal="false" class="details-container">
      <div class="details-box">
        <el-form ref="transferForm" :model="transferForm" label-width="120px" :rules="transferRules"  class="form-container">
          <div class="gray-bg">
            <div class="clearfix">
              <el-form-item label="购买人：" prop="name" class="exit-form">
                <span>{{classObj.name}}</span>
            ...
            <el-form-item label="价格：" prop="money"  style="margin-bottom: 0">
              <span>{{classObj.money}}元</span>
            </el-form-item>
            <el-form-item label="合同编号：" prop="contractNo"  style="margin-bottom: 0">
              <span>{{contractNo}}</span>
            </el-form-item>
          </div>
          <el-form-item label="转让给：" prop="memberId" style="margin-top: 20px;">
            <el-select v-model="transferForm.memberId" placeholder="输入会员姓名/手机号搜索" filterable :loading="loading" required=true>
              <el-option v-for="item in memberList" :key="item.id" :label="item.name+' '+item.phone" :value="item.id">
              </el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="销售：" prop="salesman">
            <el-select v-model="transferForm.salesman" placeholder="请选择销售" filterable>
              <el-option v-for="item in saleList" :key="item.id" :label="item.name" :value="item.id"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="备注：" prop="remark" >
          ...
          <el-form-item label="合同编号：" prop="contractNo" >
            <el-input v-model="transferForm.contractNo"></el-input>
          </el-form-item>
          <el-form-item label="转让费：" prop="num" >
            <span v-if="transferPrice == 0"><el-input v-model.number="transferForm.num" @blur="discountChange()"></el-input></span>
            <span v-else>{{transferForm.num}}&nbsp;元 <el-input v-model="transferForm.num" type="hidden"></el-input></span>
          </el-form-item>

          <el-form-item label="优惠金额：" prop="discountFee">
            <el-input v-model.number="transferForm.discountFee" placeholder="请输入金额" @blur="discountChange()"></el-input>&nbsp;&nbsp;元
          </el-form-item>

          <el-form-item label="最终结算金额：" prop="serviceFee" >

            <span>{{transferForm.serviceFee}}&nbsp;元</span>
          </el-form-item>

          <el-form-item label="支付方式：" prop="payMethod">
            <el-select v-model="transferForm.payMethod" placeholder="请选择支付方式" :clearable=false>
              <el-option v-for="item in payList" :key="item.id" :label="item.value" :value="item.id"></el-option>
            </el-select>
          </el-form-item>

          <el-form-item>
            <el-button type="primary" @click="transferSubmit">支付</el-button>
          </el-form-item>
        </el-form>
...
<script>
  ...
  var transNum,disPrice,personFee;
...
//				}
      ...
      goToClassPrint(row){
        sessionStorage.setItem("tradeId",row.id);
        sessionStorage.setItem("memberId",row.memberId)
...
//      退款
      handleExitMoney(row){
        ...
        this.exitMoneyForm.tradeId = this.classObj.id; //会员卡id
        this.memberId = this.classObj.memberId; //会员id
        this.getOrderPrint(this.classObj.id,this.classObj.memberId);
        this.$nextTick(() => {
          this.exitMoneyForm.money = this.classObj.money;
        });
        ...
        this.$refs.exitMoneyForm.validate((valid) => {
          if (valid) {
            this.$confirm('确认要退款吗？', '提示', {type: 'warning'}).then(() => {
                  ...
                  this.$axios({
                    url: this.path + 'member/card/refund',
                    method: 'post',
                    data: {
                      id: this.orderId, //订单id
                      tradeId: this.exitMoneyForm.tradeId, //会员卡id
                      type: this.exitMoneyForm.type,
                      payMethod: this.exitMoneyForm.payMethod,
                      num: this.exitMoneyForm.num,
                      remark: this.exitMoneyForm.remark,
                    },
                    transformRequest: [function (params) {
                      let ret = ''
                      for (let it in params) {
                    ...
                    if (res.data.code === 100000) {
                      ...
                      if (this.exitMoneyForm.type == 0) {
                        this.exitMoneyForm.type = "终止合同"
                      } else if (this.exitMoneyForm.type == 1) {
                        this.exitMoneyForm.type = "仅退款"
                      }
                      this.$message({
                        message: res.data.message,
                        type: 'error'
      ...
      getOrderPrint(tradeId,memberId){
        this.$axios({
          url: this.path + 'order/receipt',
          method: 'get',
          params: {
            tradeId:tradeId, //会员卡id
          ...
          .then((res) => {
            if(res.data.code === 100000) {
              ...
              this.contractNo = res.data.order.contractNo;
            }
          })
          .catch((err) => {
            console.log(err);
      ...
      handleTransfer(row){
          this.classObj = Object.assign({}, row);
          this.transferForm.id = this.classObj.id; //会员卡id
          this.getTransferPrice(this.transferForm.id);
//         this.transferForm.memberId = this.classObj.memberId; //会员id
//        this.getOrderPrint(this.classObj.id,this.classObj.memberId);
//        this.exitMoneyForm.fee = this.classObj.fee;
      ...
      getTransferPrice(cardid) {
        this.$axios({
          url: this.path + 'member/card/transfer/money?id='+cardid,
          method: 'get',
          transformRequest: [function(params) {
            let ret = ''
            for(let it in params) {
          ...
          .then((res) => {
            if(res.data.code === 100000) {
              this.transferForm.num = res.data.result; //转让费
              this.transferPrice = res.data.result;
              this.transferForm.serviceFee = this.transferForm.num;
            }else{
              this.transferForm.num = 0
              this.transferPrice = 0
              this.transferForm.serviceFee = 0
            }
          })
          .catch((err) => {
            console.log(err);
      ...
      discountChange(){
        if(this.transferForm.discountFee =='' || this.transferForm.discountFee == null){
          this.transferForm.serviceFee = this.transferForm.num
        }else{
          this.transferForm.serviceFee = this.transferForm.num - this.transferForm.discountFee;
        }
      ...
      transferSubmit(){
        this.$refs.transferForm.validate((valid) => {
          if (valid) {
        this.$confirm('确认要转让吗？', '提示', {type: 'warning'}).then(() => {

              this.$axios({
                url: this.path + 'member/card/transfer',
                method: 'post',
                data: {
                  id:this.transferForm.id, //订单id
                  memberId:this.transferForm.memberId, //会员卡id
                  payMethod: this.transferForm.payMethod,
                  discountFee: this.transferForm.discountFee, //优惠金额
                  num:this.transferForm.num,//转让费金额
                  contractNo:this.transferForm.contractNo,
                  salesman:this.transferForm.salesman,
