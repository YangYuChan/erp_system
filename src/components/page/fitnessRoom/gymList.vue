  ...
  <div class="table">
    ...
    <div class="plugins-tips">
      <el-form :inline="true" :model="listQuery" class="demo-form-inline">
        ...
        <el-form-item>
          <el-input v-model="listQuery.keyword" placeholder="输入场馆名称" clearable></el-input>
        </el-form-item>
        <el-form-item>
          <el-select v-model="listQuery.typeId" placeholder="请选择场馆类型" clearable @change="handleFilter">
            ...
            <template>
              <div id="app">
                <el-cascader
                  ...
                  @change="handleAreaChange" placeholder="请选择省/市/区" clearable>
                </el-cascader>
              </div>
            </template>
          </el-form-item>
                ...
                <el-table :data="classData" border stripe style="width:100%;margin-top:20px" v-loading="listLoading">
                  ...
                  <el-table-column prop="classKind" label="课程渠道" width="130">
                    <template slot-scope="scope">
                      ...
                      <span v-else-if="scope.row.classKind === 1">线下课程</span>
                    </template>
                  </el-table-column>
                  ...
                  <el-table-column prop="classPrice" label="售价（元）"></el-table-column>
                  <el-table-column prop="originalPrice" label="原价（元）"></el-table-column>
                  <el-table-column prop="limitPrice" label="限时折扣价（元）"></el-table-column>
                  <el-table-column prop="classStorage" label="库存"></el-table-column>
                  <el-table-column prop="classNumber" label="课时"></el-table-column>
                  <el-table-column prop="isCheck" label="审核状态">
                    <template slot-scope="scope">
                      <span v-if="scope.row.isCheck == 0">等待审核</span>
                      <span v-else-if="scope.row.isCheck == 1">审核通过</span>
                      <span v-else-if="scope.row.isCheck == 2">审核不通过</span>
                    </template>
                  </el-table-column>
      ...
      <el-dialog  :visible.sync="addFormVisible" title="添加场馆信息" width="850px" :closeOnClickModal="false">
        <el-form ref="addForm" :model="addForm" label-width="80px" :rules="addRules" class="form-container">
          ...
          <el-form-item label="场馆介绍" prop="gymInfo">
            <el-input type="textarea" v-model="addForm.gymInfo"></el-input>
          </el-form-item>
    ...
    <div class="panel">
      <el-dialog  :visible.sync="editFormVisible" title="编辑场馆信息" width="850px" :closeOnClickModal="false">
        <el-form ref="editForm" :model="editForm" label-width="80px" :rules="editRules" class="form-container">
          ...
          <div class="map-box">
            <v-editMap ref="mapEditChild" v-on:getEditMapChild="getEditMap" v-bind:map="map"></v-editMap>
          </div>
          <el-form-item label="经度" prop="longitude">
    ...
    <div class="panel">
      <el-dialog  :visible.sync="addClassVisible" title="添加课程" width="600px" :closeOnClickModal="false">
        <el-form ref="addClass" :model="addClass" label-width="120px" :rules="addClassRules" class="form-container">
          ...
          <el-form-item label="封面上传" prop="classPath">
            <el-upload class="avatar-uploader" :action="uploadUrl()"
                       :on-success="addClassPath"
                       :before-upload="beforeAvatarUpload"
                       :show-file-list="false"
                       :headers="uploadHeaders()">
              <img v-if="addClass.classPath" :src="addClass.classPath" class="avatar">
              <i v-else class="el-icon-plus avatar-uploader-icon"></i>
            </el-upload>
            <template>
              <span style="color:#999999">(点击图片重新上传)</span>
          ...
          <el-form-item label="课程售价" prop="classPrice">
            <el-input v-model.number="addClass.classPrice" :value="0" :disabled="pdisabled" ></el-input>
          </el-form-item>
          <!--v1.4-->
          <el-form-item label="课程原价" prop="originalPrice">
            <el-input v-model.number="addClass.originalPrice" :value="0"  ></el-input>
          </el-form-item>
          <div v-show="addClass.isFree === false">
            <el-form-item label="限时折扣价" prop="limitPrice">
              <el-input v-model.number="addClass.limitPrice" :value="0" ></el-input>
            </el-form-item>
            <el-form-item label="限时时间" prop="beginTime">
              <el-date-picker
                v-model="addClass.limitStartTime"
                type="datetime"
                placeholder="选择开始时间"
          ...
          <div v-show="addClass.classKind === 1">
            <el-form-item label="库存" prop="classStorage">
              <el-input v-model.number="addClass.classStorage"></el-input>
            </el-form-item>
            <el-form-item label="课程课长" prop="classTime">
              <el-input v-model.number="addClass.classTime"></el-input> min
            </el-form-item>
            <el-form-item label="包含课时" prop="classNumber">
              <el-input v-model.number="addClass.classNumber"></el-input>
            </el-form-item>
          </div>
          <!--end-->
          <!--classKind=1是线下课程 0线上课程-->
          <div v-show="addClass.classKind === 0">
            <el-form-item label="时长(秒)" prop="classVideoTime">
              <el-input v-model.number="addClass.classVideoTime" disabled="disabled" ></el-input>
            </el-form-item>
            <el-form-item label="大小(MB)" prop="classVideoSize">
              <el-input v-model.number="addClass.classVideoSize" disabled="disabled" ></el-input>
            </el-form-item>
          </div>
          <!--<el-form-item label="是否为特色课程" prop="classImportant">-->
            <!--<el-radio-group v-model="addClass.classImportant">-->
              <!--<el-radio v-for="item in classImportantData" :key="item.id" :label="item.id" :value="item.id">{{item.value}}</el-radio>-->
            <!--</el-radio-group>-->
          <!--</el-form-item>-->
          <el-form-item>
    ...
    <div class="panel">
      <el-dialog  :visible.sync="editClassVisible" title="编辑课程" width="600px" :closeOnClickModal="false">
        <el-form ref="editClass" :model="editClass" label-width="120px" :rules="editClassRules" class="form-container">
          <el-form-item label="课程名称" prop="className">
          ...
          <el-form-item label="封面上传" prop="classPath">
            <el-upload class="avatar-uploader" :action="uploadUrl()"
                       :on-success="editClassPath"
                       :before-upload="beforeAvatarUpload"
                       :show-file-list="false"
                       :headers="uploadHeaders()">
              <img v-if="editClass.classPath" :src="editClass.classPath" class="avatar">
              <i v-else class="el-icon-plus avatar-uploader-icon"></i>
            </el-upload>
            <template>
              <span style="color:#999999">(点击图片重新上传)</span>
          ...
          <div v-if="editClass.isFree === false">
          <el-form-item label="课程售价" prop="classPrice">
            <el-input v-model.number="editClass.classPrice"></el-input>
          </el-form-item>
          </div>
          <div v-else>
            <el-form-item label="课程售价" prop="classPrice">
              <el-input v-model.number="editClass.classPrice" :disabled="epdisabled" ></el-input>
            </el-form-item>
          </div>
          <!--v1.4-->
          <el-form-item label="课程原价" prop="originalPrice">
            <el-input v-model.number="editClass.originalPrice"  ></el-input>
          </el-form-item>
          <div v-show="editClass.isFree === false">
            <el-form-item label="限时折扣价" prop="limitPrice">
              <el-input v-model.number="editClass.limitPrice" ></el-input>
            </el-form-item>
            <el-form-item label="限时时间" prop="beginTime">
              <el-date-picker
                v-model="editClass.limitStartTime"
                type="datetime"
                placeholder="选择开始时间"
                value-format="yyyy-MM-dd HH:mm:ss"
                format="yyyy-MM-dd HH:mm:ss"
                style="margin-bottom: 10px"
                :picker-options="pickerEditDateBefore"
              >
              </el-date-picker>
              <el-date-picker
                v-model="editClass.limitEndTime"
                type="datetime"
                value-format="yyyy-MM-dd HH:mm:ss"
                format="yyyy-MM-dd HH:mm:ss"
                placeholder="选择结束时间"
                :picker-options="pickerEditDateAfter"
              >
              </el-date-picker>
            </el-form-item>
          </div>
          <div v-show="editClass.classKind === 1">
            <el-form-item label="库存" prop="classStorage">
              <el-input v-model.number="editClass.classStorage"></el-input>
            </el-form-item>
            <el-form-item label="课程课长" prop="classTime">
              <el-input v-model.number="editClass.classTime"></el-input> min
            </el-form-item>
            <el-form-item label="包含课时" prop="classNumber">
              <el-input v-model.number="editClass.classNumber"></el-input>
            </el-form-item>
          </div>
          <!--end-->
          <!--classKind=1是线下课程 0线上课程-->
          <div v-show="editClass.classKind === 0">
            <el-form-item label="时长(秒)" prop="classVideoTime">
              <el-input v-model.number="editClass.classVideoTime" disabled="disabled" ></el-input>
            </el-form-item>
            <el-form-item label="大小(MB)" prop="classVideoSize">
              <el-input v-model.number="editClass.classVideoSize" disabled="disabled" ></el-input>
            </el-form-item>
          </div>
          <!--<el-form-item label="是否为特色课程" prop="classImportant">-->
            <!--<el-radio-group v-model="editClass.classImportant">-->
              <!--<el-radio v-for="item in classImportantData" :key="item.id" :label="item.id" :value="item.id">{{item.value}}</el-radio>-->
            <!--</el-radio-group>-->
          <!--</el-form-item>-->
          <el-form-item>
...
<script>
  ...
  import { regionData, CodeToText, TextToCode} from '../../../common/js/area.js'
  import AMap from 'AMap';
  import vMap from './Map.vue';
  import vEditMap from './editMap.vue';
  import $ from 'jquery';
  var normalPrice,normalEditPrice,addStartTime;
    ...
    data() {
      return {
        ...
        pickerBeginDateBefore:{
          disabledDate: (time) => {
            let end = new Date(this.addClass.limitEndTime).getTime();
            if(end){
//              return time.getTime() > (end-24*60*60*1000); //之前一天
        ...
        pickerBeginDateAfter:{
          disabledDate: (time) => {
            let start = new Date(this.addClass.limitStartTime).getTime();
            if(start){
//              return time.getTime() < (start+24*60*60*1000); //后面一天
              return time.getTime() < start;
            }
          }
        },
        pickerEditDateBefore:{
          disabledDate: (time) => {
            let end = new Date(this.editClass.limitEndTime).getTime();
            if(end){
//              return time.getTime() > (end-24*60*60*1000); //之前一天
              return time.getTime() > end ;
            }
        ...
        pickerEditDateAfter:{
          disabledDate: (time) => {
            let start = new Date(this.editClass.limitStartTime).getTime();
            if(start){
//              return time.getTime() < (start+24*60*60*1000); //后面一天
              return time.getTime() < start;
            }
        ...
        addClass: {
          ...
          isFree: true,
          classPrice: 0,
          isCheck:0,//默认等待审核(v1.4)
          originalPrice:'',// 真实价格(v1.4)
          limitPrice:'',// 限时特价(v1.4)
          limitStartTime:'',// 限时特价开始时间(v1.4)
          limitEndTime:'',// 限时特价结束时间(v1.4)
          classNumber:'',// 课时(v1.4)
          classStorage:'',// 库存(v1.4)
          classTime:'',// 时长(v1.4)
          classPath:'',
          classImportant:0
        },
        ...
        addClassRules: {
          ...
          originalPrice:[{
            ...
            validator:function (rule, value, callback) {
              if(value.length == 0){
                return callback(new Error('请输入课程原价'));
              }else{
                if(value < 0){
                  return callback(new Error('课程原价不能小于0'));
                }else if(value < normalPrice) {
                  return callback(new Error('课程原价必须大于售价'));
                }else{
                  callback();
                }
              }
          ...
          limitPrice:[{
            ...
            validator:function (rule, value, callback) {
              if(value.length == 0){
                return callback(new Error('请输入限时折扣价'));
              }else{
                if(value < 0){
                  return callback(new Error('限时折扣价不能小于0'));
                }else if(value > normalPrice) {
                  return callback(new Error('限时折扣价必须小于售价'));
                }else{
                  callback();
                }
              }
            ...
            this.$confirm('您确定要提交吗?', '提示', {
              ...
              this.$axios({
                ...
                data: {
                  ...
                  closingTime:this.addForm.closingTime,
            ...
            this.$confirm('您确定要提交吗?', '提示', {
              ...
              this.$axios({
                ...
                data: {
                  ...
                  closingTime:this.editForm.closingTime,
                },
                transformRequest: [function (data) {
                  let ret = ''
                  for (let it in data) {
...
//                  this.selectDetails(this.gymId);
      ...
      FreeChange() {
        ...
        if (free == true) {
          ...
          this.addClass.limitPrice = 0;
          this.pdisabled = true;
        } else {
          this.addClass.classPrice = '';
          this.addClass.limitPrice = '';
      ...
      eFreeChange() {
        ...
        if (efree == true) {
          ...
          this.editClass.limitPrice = 0;
          this.epdisabled = true;
        } else {
          this.editClass.classPrice = this.editClass.classPrice;
          this.editClass.limitPrice = this.editClass.limitPrice;
          ...
          if((this.addClass.isFree == false)&&(this.addClass.classPrice <= 0 || this.addClass.classPrice == null)){
            ...
            if (valid) {
              this.$confirm('您确定要提交吗?', '提示', {
                ...
                if(this.addClass.classPrice == null){
                  this.addClass.classPrice = 0;
                }
                if(this.addClass.originalPrice == null){
                  this.addClass.originalPrice = '';
                }
                if(this.addClass.limitPrice == null){
                  this.addClass.limitPrice = '';
                }
                if(this.addClass.classNumber == null){
                  this.addClass.classNumber = '';
                }
                if(this.addClass.classStorage == null){
                  this.addClass.classStorage = '';
                }
                if(this.addClass.classTime == null){
                  this.addClass.classTime = '';
                }
                if(this.addClass.limitStartTime == null){
                  this.addClass.limitStartTime = '';
                }
                if(this.addClass.limitEndTime == null){
                  this.addClass.limitEndTime = '';
                }
                this.$axios({
                  ...
                  data: {
                    ...
                    isCheck:0,// 默认等待审核(v1.4)
                    originalPrice:this.addClass.originalPrice,// 真实价格(v1.4)
                    limitPrice:this.addClass.limitPrice,// 限时特价(v1.4)
                    limitStartTime:this.addClass.limitStartTime,// 限时特价开始时间(v1.4)
                    limitEndTime:this.addClass.limitEndTime,// 限时特价结束时间(v1.4)
                    classNumber:this.addClass.classNumber,// 课时(v1.4)
                    classStorage:this.addClass.classStorage,// 库存(v1.4)
                    classTime:this.addClass.classTime,// 时长(v1.4)
                    classImportant:this.addClass.classImportant,
                    classPath:this.addClass.classPath
                  },
                  transformRequest: [function (data) {
                    let ret = ''
                    for (let it in data) {
          ...
          if((this.editClass.isFree == false)&&(this.editClass.classPrice <= 0 || this.editClass.classPrice == null)){
            ...
            if (valid) {
              this.$confirm('您确定要提交吗?', '提示', {
                ...
                if(this.editClass.classPrice == null){
                  this.editClass.classPrice = 0;
                }
                if(this.editClass.originalPrice == null){
                  this.editClass.originalPrice = '';
                }
                if(this.editClass.limitPrice == null){
                  this.editClass.limitPrice = '';
                }
                if(this.editClass.classNumber == null){
                  this.editClass.classNumber = '';
                }
                if(this.editClass.classStorage == null){
                  this.editClass.classStorage = '';
                }
                if(this.editClass.classTime == null){
                  this.editClass.classTime = '';
                }
                if(this.editClass.limitStartTime == null){
                  this.editClass.limitStartTime = '';
                }
                if(this.editClass.limitEndTime == null){
                  this.editClass.limitEndTime = '';
                }
                this.$axios({
                  ...
                  data: {
                    ...
                    originalPrice:this.editClass.originalPrice,// 真实价格(v1.4)
                    limitPrice:this.editClass.limitPrice,// 限时特价(v1.4)
                    limitStartTime:this.editClass.limitStartTime,// 限时特价开始时间(v1.4)
                    limitEndTime:this.editClass.limitEndTime,// 限时特价结束时间(v1.4)
                    classNumber:this.editClass.classNumber,// 课时(v1.4)
                    classStorage:this.editClass.classStorage,// 库存(v1.4)
                    classTime:this.editClass.classTime,// 时长(v1.4)
                    classImportant:this.editClass.classImportant,
                    classPath:this.editClass.classPath
                  },
                  transformRequest: [function (data) {
                    let ret = ''
                    for (let it in data) {
      ...
      handleRemoveClass(row) {
        ...
        this.$confirm('是否下架'+row.className+'<br/>提示：下架后该课程不会在app上显示并被删除',{
          cancelButtonText: '取消',
      ...
      addClassPath(res, file){
        this.addClass.classPath = res;
      },
      editClassPath(res, file){
        this.editClass.classPath = res;
      },
      //上传环境图片
